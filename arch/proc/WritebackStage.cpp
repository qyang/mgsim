...
Processor::Pipeline::PipeAction Processor::Pipeline::WritebackStage::OnCycle()
{
    ...
    if (m_input.Rrc.type != RemoteMessage::MSG_NONE)
    {
        ...
        DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s sent network message %s",
    ...
    if (m_input.Rcv.m_state != RST_INVALID)
    {
        ...
        if (writebackOffset < size)
        {
            ...
            if (m_input.Rc.valid())
            {
                ...
                {
                    DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to acquire write port on RF",
                ...
                {
                    DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to read prev %s",
                                  (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                  m_input.pc_sym,
                                  addr.str().c_str());
                ...
                if (value.m_state == RST_WAITING)
                {
                    ...
                    if (old_value.m_state == RST_FULL)
                    {
                        ...
                        DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s data arrived in %s since read, rescheduling",
                                      (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                       m_input.pc_sym,
                                       addr.str().c_str());
                    }
                ...
                else if (value.m_state == RST_PENDING && old_value.m_state == RST_WAITING)
                {
                    ...
                    DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s combining pending and waiting -> %s",
                                   (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                   m_input.pc_sym,
                ...
                else if (old_value.m_state == RST_EMPTY || old_value.m_state == RST_PENDING)
                {
                    if (old_value.m_memory.size != 0)
                    {
                        ...
                        DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s target busy %s %s, retrying",
                                       (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                       m_input.pc_sym,
                ...
                if (value.m_state != RST_INVALID)
                {
                    ...
                    {
                        DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to write %s <- %s",
                                      (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                      m_input.pc_sym,
                    ...
                    DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s writeback %s <- %s",
                                   (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                   m_input.pc_sym,
                                   addr.str().c_str(), value.str(addr.type).c_str());
                }
    ...
    else if (m_input.suspend == SUSPEND_MEMORY_BARRIER)
    {
        ...
        DebugPipeWrite("F%u/T%u(priority:%u   index :%llu) %s memory barrier, suspend: %s",
                       (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                       m_input.pc_sym,
                       suspend ? "yes" : "no");
    ...
    if (writebackOffset == size)
    {
        ...
        if (m_input.swch)
        {
            ...
            if (m_input.kill)
            {
                ...
                {
                    DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to terminate thread",
                                  (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                  m_input.pc_sym);
                    return PIPE_STALL;
                }
            ...
            else if (suspend)
            {
                ...
                {
                    DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to suspend thread",
                                  (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                  m_input.pc_sym);
                    return PIPE_STALL;
                }
            ...
            else if (allow_reschedule)
            {
                ...
                if (!m_allocator.RescheduleThread(m_input.tid, m_input.pc))
                {
                    ...
                    DeadlockWrite("F%u/T%u(priority:%u   index :%llu) %s unable to reschedule thread",
                                  (unsigned)m_input.fid, (unsigned)m_input.tid, (unsigned)m_input.priority, (unsigned long long)m_input.logical_index,
                                  m_input.pc_sym);
                    return PIPE_STALL;
                }
