...
void SimpleMemory::RegisterListener(PSize /*pid*/, IMemoryCallback& callback, const ArbitrationSource* /*sources*/)
{
    m_caches.insert(&callback);
...
void SimpleMemory::UnregisterListener(PSize /*pid*/, IMemoryCallback& callback)
{
...
SimpleMemory::SimpleMemory(Object* parent, Kernel& kernel, const std::string& name, const Config& config) :
    ...
    m_requests  (kernel, config.getInteger<BufferSize>("CMBufferSize", INFINITE))
{
