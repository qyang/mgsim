...
void Object::OutputWrite_(const char* msg, ...) const
{
    ...
    fprintf(stderr, "[%08llu:%s]\t\to ", (unsigned long long)m_kernel.GetCycleNo(), GetFQN().c_str());
    va_start(args, msg);
...
void Object::DeadlockWrite_(const char* msg, ...) const
{
    ...
    fprintf(stderr, "[%08llu:%s]\t(%s)\td ", (unsigned long long)m_kernel.GetCycleNo(), GetFQN().c_str(), m_kernel.GetActiveProcess()->GetName().c_str());
    va_start(args, msg);
    vfprintf(stderr, msg, args);
    va_end(args);
    fputc('\n', stderr);
...
void Object::DebugSimWrite_(const char* msg, ...) const
{
    ...
    fprintf(stderr, "[%08llu:%s]\t", (unsigned long long)m_kernel.GetCycleNo(), GetFQN().c_str());
